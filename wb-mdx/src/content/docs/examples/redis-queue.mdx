...
<Tabs>
  ...
  <TabItem label="redis-config.yml">
    ...
    See the [Redis module](/winter-boot/modules/data-redis) for the connection properties.
  </TabItem>
</Tabs>
...
- Read the [Redis module](/winter-boot/modules/data-redis) for the connection templates (`PhpRedisTemplate`, cluster, sentinel) behind the queue consumers.
- Compare with the [SQS consumer](/winter-boot/examples/sqs-consumer) and [Kafka consumer](/winter-boot/examples/kafka-app): same auto-start worker shape, different delivery guarantees.
- Browse all [Libraries](/winter-boot/modules/overview) when you need S3, OpenSearch, or Doctrine in the same app.
