...
Use a daemon for work that must always be running — watching a directory, polling a queue, or tailing a stream. Use [scheduled tasks](/winter-boot/examples/scheduler-app) instead when the work runs on a fixed cadence.
...
- Read [Daemon Threads](/winter-boot/async/daemon-threads) for process types, `coreSize` scaling, and supervision details.
- Read the [Scheduler example](/winter-boot/examples/scheduler-app) when your work runs on a fixed cadence instead of continuously.
