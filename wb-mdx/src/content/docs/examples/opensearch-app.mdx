...
<Tabs>
  ...
  <TabItem label="opensearch-config.yml">
    ...
    Replace the host, username, and password with your own cluster credentials. The `__default__` entry supplies shared defaults. See the [OpenSearch module](/winter-boot/modules/opensearch) for every connection property.
  </TabItem>
</Tabs>
...
- Read the [OpenSearch module](/winter-boot/modules/opensearch) for the full `OpenSearchTemplate` API and AWS SigV4 signing.
