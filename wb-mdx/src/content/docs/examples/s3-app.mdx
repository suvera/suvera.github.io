...
<Tabs>
  ...
  <TabItem label="s3-config.yml">
    ...
    The `__default__` entry supplies shared defaults. See the [S3 module](/winter-boot/modules/s3) for every connection property.
  </TabItem>
</Tabs>
...
- Read the [S3 module](/winter-boot/modules/s3) for the full `S3Template` API and IAM-role setup.
