...
<Tabs>
  ...
  <TabItem label="kafka-config.yml">
    ...
    The `__default__` entry supplies shared defaults. Replace `bootstrap.servers` with your own brokers outside local testing. See the [Kafka module](/winter-boot/modules/kafka) for every consumer property and producer setup.
  </TabItem>
</Tabs>
...
- Read the [Kafka module](/winter-boot/modules/kafka) for producer APIs (`KafkaService`), consumer tuning, and rdkafka settings.
- Browse all [Libraries](/winter-boot/modules/overview) when you need SQS, S3, Redis, or Doctrine in the same app.
