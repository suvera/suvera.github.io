...
Winter S3 is a module that provides easy configuration and access to S3 object storage (or S3-compatible services) from Winter Boot applications. Each named entry in `s3-config.yml` builds an `Aws\S3\S3Client` and registers an `S3Template` bean, so you can talk to multiple buckets, regions, or accounts from the same service. If you already use the [SQS](/winter-boot/modules/sqs) module, the configuration patterns here, like named connections and IAM-role credentials, will feel familiar.
...
<Aside type="tip">
  For AWS-managed queues alongside object storage, see the [SQS](/winter-boot/modules/sqs) module.
</Aside>
